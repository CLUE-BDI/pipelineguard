...
# Bypass python keyring to prevent "Please enter your password:" prompts
export PYTHON_KEYRING_BACKEND=keyring.backends.null.Keyring

# GCS Cleanup
GCS_BUCKET="pipelineguard-artifacts"
echo "Removing from GCS bucket: ${GCS_BUCKET}..."
gcloud storage rm -r "gs://${GCS_BUCKET}/raw/*/*/*/vulnerable-infra-project" 2>/dev/null || echo "No raw files to delete or deletion failed."
gcloud storage rm -r "gs://${GCS_BUCKET}/normalized/*/*/*/vulnerable-infra-project" 2>/dev/null || echo "No normalized files to delete or deletion failed."
gcloud storage rm -r "gs://${GCS_BUCKET}/correlated/*/*/*/vulnerable-infra-project" 2>/dev/null || echo "No correlated files to delete or deletion failed."
